...
internal sealed class BddEnvironment
{
    ...
    public async Task EnsureInitializedAsync()
    {
        ...
        _createScriptPath = Path.Combine(repoRoot, "DB_data_scripts", "integration", "create.sql");
        _truncateScriptPath = Path.Combine(repoRoot, "DB_data_scripts", "integration", "truncate.sql");
        _copyScriptPath = Path.Combine(repoRoot, "DB_data_scripts", "integration", "copy_all.sql");
    ...
    {
        if (!_initialized)
            throw new InvalidOperationException("BDD environment is not initialized.");
    ...
    public async Task ForcePasswordExpiredAsync(string email)
    {
        await using var context = CreateDbContext();
        await context.Database.ExecuteSqlInterpolatedAsync($@"
            update users
            set last_password_changed_at_utc = now() - interval '365 days',
                password_change_required = true
            where email = {email}");
    }

    public async Task ForceLockoutExpiredAsync(string email)
    {
        await using var context = CreateDbContext();
        await context.Database.ExecuteSqlInterpolatedAsync($@"
            update users
            set lockout_until_utc = now() - interval '1 minute'
            where email = {email}");
    }
    ...
    private static string ResolveBaseUrl()
    {
        ...
        if (string.IsNullOrWhiteSpace(raw))
            throw new InvalidOperationException($"{BaseUrlEnv} is not set. BDD E2E tests are external-only.");
    ...
    private async Task WaitForDatabaseReadyAsync()
    {
        ...
        throw new InvalidOperationException(
            $"BDD E2E database is not reachable using env '{DbHostEnv}:{DbHostPortEnv}/{DbNameEnv}'. Last error: {lastError}");
    }
    ...
    private async Task EnsureExternalApplicationIsReachableAsync()
    {
        ...
        throw new InvalidOperationException(
            $"BDD E2E app at '{BaseApiUrl}' is not reachable. Set {BaseUrlEnv} to running app-under-test URL.",
            lastException);
    }
}
