...
docker compose -p "$COMPOSE_PROJECT_NAME" "${COMPOSE_ARGS[@]}" run --rm integration-tests dotnet test src/Tests/Project.Integration.Tests/Project.Integration.Tests.csproj --configuration Release --no-build --logger "trx;LogFileName=integration-tests.trx" --results-directory TestResults "$@" -- xUnit.ReporterSwitch=allure
